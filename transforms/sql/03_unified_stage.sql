-- transforms/sql/03_unified_silver.sql
-- Unified Silver view (evidence-preserving, no cross-source dedupe)
-- Combines silver.jobspy, silver.profesia_sk, silver.stepstone into one schema.
-- Canonicalizes URLs/hosts, normalizes company names where missing, and aligns contacts.
...
WITH jobspy_rows AS (
  SELECT
    ...
    -- URLs & hosts
    -- job_url_raw prefers job_url_direct then source_row_url
    ...
    (regexp_match(
       COALESCE(util.url_canonical(COALESCE(j.job_url_direct, j.source_row_url)),
                util.url_canonical(j.source_row_url),
                ''),
       '/jobs/view/([0-9]+)'))[1]                                           AS linkedin_job_id,
    -- apply_url_raw: prefer direct, else job_url_raw
    COALESCE(j.job_url_direct, COALESCE(j.job_url_direct, j.source_row_url))::text AS apply_url_raw,
    ...
    util.company_name_norm_langless(COALESCE(j.company_name, j.company_raw)) AS company_name_norm_langless,
    util.company_name_norm(COALESCE(j.company_name, j.company_raw))          AS company_name_norm,

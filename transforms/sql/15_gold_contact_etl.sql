...
unified AS (
  SELECT
    ...
    us.contacts_raw                    AS contacts_raw,  -- already JSONB in unified
...
-- ======================
-- Resolve to gold.company (stepstone -> domain -> same_org -> slug)
-- ======================
company_resolved AS (
  SELECT
    h.*,
    COALESCE(
      -- 1) StepStone id -> company
      (
...
-- ======================
-- ATOMS from contacts_raw (StepStone-style)
-- ======================
json_atoms AS (
  SELECT
  ...
  LEFT JOIN LATERAL jsonb_array_elements(
    CASE
      WHEN cr.contacts_raw IS NOT NULL
           AND jsonb_typeof(cr.contacts_raw) = 'array'
        THEN cr.contacts_raw
      ELSE '[]'::jsonb
    END
  ) AS c(value) ON TRUE
),
...
-- ======================
-- ATOMS from emails_raw (split common separators)
-- ======================
emails_raw_atoms AS (
  SELECT
    cr.source, cr.source_id, cr.source_row_url, cr.scraped_at, cr.date_posted,
...
-- ======================
-- Re-resolve company from email_root if no hint (exclude aggregator/ATS/career)
-- ======================
...
existing_by_email AS (
  SELECT DISTINCT
    a.source, a.source_id, a.email, ce.contact_id
  FROM atoms_eligible a
  JOIN gold.contact_evidence ce
    ON ce.kind = 'email' AND ce.value = a.email
  WHERE a.email IS NOT NULL
),
existing_by_name_company AS (
  SELECT DISTINCT
    a.source, a.source_id, a.name_norm, a.company_id, ca.contact_id
  FROM atoms_eligible a
  JOIN gold.contact_alias ca
    ON ca.alias_norm = a.name_norm
  JOIN gold.contact_affiliation aff
    ON aff.contact_id = ca.contact_id
   AND aff.company_id = a.company_id
  WHERE a.name_norm IS NOT NULL AND a.company_id IS NOT NULL
),
existing_by_phone_company AS (
  SELECT DISTINCT
    a.source, a.source_id, a.phone_norm, a.company_id, ce.contact_id
  FROM atoms_eligible a
  JOIN gold.contact_evidence ce
    ON ce.kind = 'phone' AND ce.value = a.phone_norm
  JOIN gold.contact_affiliation aff
    ON aff.contact_id = ce.contact_id
   AND aff.company_id = a.company_id
  WHERE a.phone_norm IS NOT NULL AND a.company_id IS NOT NULL
),

-- ======================
-- Seed keys (stable per atom)
-- ======================
seeds AS (
  SELECT
    a.*,
...
-- ======================
-- Map atoms to existing contact ids (if any)
-- ======================
atoms_mapped AS (
  SELECT
    s.*,
...
-- ======================
-- Best display per seed (for inserts)
-- ======================
seed_best AS (
  SELECT
    seed_key,
...
ins_contacts AS (
  ...
  SELECT
    ...
    (
      ...
      ORDER BY (s2.email = sb.best_email) DESC NULLS LAST,
               s2.scraped_at DESC
      LIMIT 1
    )
  FROM seed_best sb
...
-- ======================
-- Final mapping: every atom -> contact_id
-- ======================
atom_contact AS (
  SELECT
...
ins_aliases AS (
  ...
  JOIN atoms_eligible a
    ON a.source = ac.source AND a.source_id = ac.source_id
  WHERE a.person_name IS NOT NULL
  ON CONFLICT (contact_id, alias_norm) DO NOTHING
  RETURNING 1
),

-- Ensure one primary alias (prefer best_name per seed)
...
ins_ev_phone AS (
  ...
  SELECT DISTINCT
    ac.contact_id, 'phone', a.phone_norm, a.source, a.source_id
  FROM atom_contact ac
  JOIN atoms_with_company a
    ON a.source = ac.source AND a.source_id = ac.source_id
...
ins_ev_name AS (
  INSERT INTO gold.contact_evidence (contact_id, kind, value, source, source_id)
  SELECT DISTINCT
    ac.contact_id, 'name', a.name_norm, a.source, a.source_id
  FROM atom_contact ac
  JOIN atoms_with_company a
    ON a.source = ac.source AND a.source_id = ac.source_id
...
ins_ev_title AS (
  INSERT INTO gold.contact_evidence (contact_id, kind, value, source, source_id)
  SELECT DISTINCT
    ac.contact_id, 'title', a.title, a.source, a.source_id
  FROM atom_contact ac
  JOIN atoms_with_company a
    ON a.source = ac.source AND a.source_id = ac.source_id
...
ins_ev_row AS (
  INSERT INTO gold.contact_evidence (contact_id, kind, value, source, source_id, detail)
  SELECT DISTINCT
    ac.contact_id, 'source_row', a.source_row_url, a.source, a.source_id,
...
ins_ev_company_hint AS (
  INSERT INTO gold.contact_evidence (contact_id, kind, value, source, source_id)
  SELECT DISTINCT
    ac.contact_id, 'company_hint',
    COALESCE(a.company_root, a.email_root),
    a.source, a.source_id
  FROM atom_contact ac
  JOIN atoms_with_company a
    ON a.source = ac.source AND a.source_id = ac.source_id
...
-- ======================
-- Affiliations (roll first/last_seen, compute active)
-- ======================
aff_src AS (
  SELECT
    ac.contact_id,
...
promote_primary AS (
  ...
  SET
    -- primary_email: prefer match to active affiliation root; else any non-generic; else first seen
    primary_email = COALESCE(
      (
        SELECT ce.value
        FROM gold.contact_evidence ce
        JOIN gold.contact_affiliation ca
          ON ca.contact_id = c.contact_id AND ca.active = TRUE
        JOIN gold.company gc
          ON gc.company_id = ca.company_id
    ...
    -- primary_phone: longest normalized
    primary_phone = (
      SELECT ce.value
      FROM gold.contact_evidence ce
    ...
    -- primary_company_id: tied to primary_email if possible; else most recent affiliation

...
unified AS (
  SELECT
    ...
    util.jsonb_safe(us.contacts_raw)                     AS contacts_raw,
...
-- Choose a seed key for new inserts (stable per atom)
seeds AS (
  SELECT
    a.*,
    CASE
...
-- Map to an existing contact_id if any
atoms_mapped AS (
  SELECT
    s.*,
...
-- For inserts: pick a "best display" per seed
seed_best AS (
  SELECT
    seed_key,
...
-- Insert new contacts where no existing
ins_contacts AS (
...
atom_contact AS (
  SELECT
    ...
    COALESCE(am.contact_id_existing,
...
-- Ensure one primary alias per contact (prefer best_name per seed)
...
-- Evidence: emails (ALL, incl. generic)
ins_ev_email AS (
...
-- Evidence: titles (from structured json if present)
ins_ev_title AS (
  INSERT INTO gold.contact_evidence (contact_id, kind, value, source, source_id)
  SELECT DISTINCT
...
-- Evidence: company hints (email_root or company_root)
...
-- Affiliations (one per contact/company; roll first/last_seen; compute active)
aff_src AS (
  SELECT
    ac.contact_id,
    a.company_id,
...
-- Promote primaries on contact
promote_primary AS (
  UPDATE gold.contact c
  SET
    -- primary_email: prefer one whose root matches an active affiliation; else any non-generic; else first seen email
    ...
    -- primary company: tied to the chosen primary_email; else most frequent affiliation; else most recent

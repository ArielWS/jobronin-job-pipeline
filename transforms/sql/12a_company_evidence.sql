-- Build domain evidence from either company_website or company_domain
-- and skip aggregators/ATS/career hosts.
...
ev AS (
  ...
  FROM m
  WHERE company_id IS NOT NULL
    AND email_root IS NOT NULL
  UNION ALL
  SELECT company_id, 'apply', apply_root, source, source_id
  FROM m
  WHERE company_id IS NOT NULL
    AND apply_root IS NOT NULL
